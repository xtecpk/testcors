...
function InventoryStaff() {
  ...
  const [totalInventoryStaff, setTotalInventoryStaff] = useState<number>(0);
  const [activeInventoryStaff, setActiveInventoryStaff] = useState<number>(0);
  ...
  useEffect(() => {
    const fetchSummaryData = async () => {
      try {
        ...
        setTotalInventoryStaff(data.totalInventoryStaff);
        setActiveInventoryStaff(data.activeInventoryStaff);
        setOnLeave(data.onLeave);
        setNewInventoryStaff(data.newInventoryStaff);
      } catch (error) {
    ...
    <>
      ...
      <div className="container-fluid ">
        <div className="row gap-12 px-5 ">
          ...
          Total Inventory Staff <br />
            <strong className="mt-1">{totalInventoryStaff}</strong>
          </div>
          <div className="col flex flex-col w-10/12 h-32 m-2 p-6 rounded-lg cursor-pointer transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 bg-white text-start inter text-lg font-semibold">
          Active Inventory Staff <br />
            <strong className="mt-1">{activeInventoryStaff}</strong>
          </div>
          <div className="col flex flex-col w-10/12 h-32 m-2 p-6 rounded-lg cursor-pointer transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 bg-white text-start inter text-lg font-semibold">
            On Leave <br />
          ...
          <div className="col flex flex-col w-10/12 h-32 m-2 p-6 rounded-lg cursor-pointer transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 bg-white text-start inter text-lg font-semibold">
          New Inventory Staff <br />
            <strong className="mt-1">{newInventoryStaff}</strong>
          </div>
        </div>
      </div>
