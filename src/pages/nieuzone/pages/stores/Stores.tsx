...
import StoresModal from "./modal/StoresModal";
...
function Stores() {
  const [rowData, setRowData] = useState<StoresData[]>([]);
  ...
  const columnDefs: ColDef[] = [
    ...
    { field: "section", flex: 1, headerName: "Section", editable: true, filter: "agTextColumnFilter", floatingFilter: true },
    { field: "name", flex: 1, headerName: "Name", editable: true, filter: "agTextColumnFilter", floatingFilter: true },
    { field: "incharge", flex: 1, headerName: "Incharge", editable: false, filter: "agTextColumnFilter", floatingFilter: true },
  ...
  const handleCellEdit = async (params: CellEditingStoppedEvent) => {
    const updatedData = params.data as StoresData;
    try {
  ...
  return (
    <>
      ...
      <StoresModal show={showDetails} onHide={() => setShowDetails(false)} />
    </>
  );
}
